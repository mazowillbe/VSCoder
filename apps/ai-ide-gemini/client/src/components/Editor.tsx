...
export default function Editor() {
  ...
  useEffect(() => {
    const syncToWebContainer = async () => {
      if (isReady && currentFile && content) {
        try {
          // WebContainer sync happens automatically
          console.log(`Syncing ${currentFile} to WebContainer`);
        } catch (error) {
    ...
    // Debounce sync to avoid too frequent writes
    const timer = setTimeout(() => {
      syncToWebContainer();
    }, 500);

    return () => clearTimeout(timer);
  }, [isReady, currentFile, content]);
  ...
  const handleEditorSave = async () => {
    // Save functionality handled by WebContainer sync
    console.log('File save triggered - WebContainer will sync automatically');
  };
